# Glossário — Portuguese legal terms

Short, alphabetical glossary of Portuguese legal terms used throughout
this directory. Focused on terms a non-Brazilian reader (or a language
model without deep Brazilian context) is likely to misread or need
disambiguation for. Definitions are short — one or two lines — with a
pointer to the file where the term is discussed in depth when relevant.

For acronyms (CGU, MPF, FPM, etc.), see [`siglas.md`](siglas.md).
For statutes by number, see [`leis_index.yaml`](leis_index.yaml).

**Topics / keywords**: glossário, terminologia jurídica, legal
terminology, definitions, Portuguese.

---

## A

- **acórdão** — Written collegiate judgment of a tribunal. Issued by a turma, câmara, plenário, or full court — multiple judges. Contrast with *decisão monocrática* (single-judge decision).
- **ação civil pública** — Collective-interest civil action regulated by Lei 7.347/1985. Can be filed by MP, public entities, qualifying associations, and others. The dominant procedural vehicle for environmental, consumer, public-patrimony, and improbidade matters.
- **ação popular** — Citizen suit (Lei 4.717/1965). Any citizen has standing to challenge an act lesivo ao patrimônio público. Narrower than ACP — focused on annulment of administrative acts.
- **ação rescisória** — Action to rescind a final judgment after trânsito em julgado. Extraordinary remedy with specific grounds (CPC Art. 966); strict deadlines.
- **agravo** — Interlocutory appeal. Several flavors: agravo de instrumento (immediate, sent to the higher court), agravo retido (held until final appeal), agravo interno (within the same court).
- **antiguidade** — Seniority. One of two criteria for career advancement in the judiciary and MP (the other is *merecimento*).
- **apelação** — Ordinary appeal from a first-instance decision (sentença) to the corresponding tribunal.
- **arguição** — Formal challenge or motion. Arguição de descumprimento de preceito fundamental (ADPF) is the STF action of that name.
- **audiência** — Hearing. *Audiência inaugural / de conciliação*: opening hearing. *Audiência de instrução e julgamento*: evidence-and-judgment hearing.
- **auto / autos** — Case file. "Os autos" = the case record.
- **autoria** — Authorship of a criminal act; identifies who committed the conduct (vs. *materialidade*, which proves that the act occurred).

## B

- **baixa / baixar** — Administrative closing of a case in the court system. A case "baixado" is closed but may not yet be definitively archived.

## C

- **caixa dois** — Off-the-books cash. Refers to slush funds, undeclared campaign money, parallel accounting. The legal/illegal split is *caixa um* (declared, legal) vs. *caixa dois* (undeclared, often illegal).
- **câmara municipal** — Municipal council; local legislative body. Vereadores serve here.
- **caput** — The opening text of a statutory article — the "head" — before any § (parágrafo) or roman-numeral subdivisions.
- **cartório** — Notary office; also used for the registry/clerk's office of a court vara.
- **cassação** — Loss of mandate via revocation. For elected officials, can be triggered by Ficha Limpa, electoral abuse, or improbidade.
- **causa de pedir** — Cause of action; the legal-factual basis for a claim.
- **coisa julgada** — Res judicata. *Trânsito em julgado* refers to the moment a decision becomes coisa julgada (no more appeals).
- **coligação** — Electoral coalition. For majoritário offices, still permitted. For proportional offices, abolished by EC 97/2017 from 2020 elections onward; partially replaced by *federações partidárias*.
- **comarca** — Judicial district at the state-court level. The basic territorial unit of the Justiça Estadual.
- **concurso público** — Competitive public examination for civil-service positions. The standard entry path into the judiciary, MP, public administration. *Concurso* with no qualifier almost always means a hiring competition (not the procurement *concurso* modality).
- **conexão** — Procedural connection between cases (same parties, same cause, related facts) that justifies joint handling. Distinguish from *prevenção*.
- **conselheiro** — A member of a TC (TCU/TCE/TCM) or of CNJ/CNMP. Not a judge in the strict sense.
- **contas** — Accounts. *Contas de governo*: a mayor/governor/president's annual consolidated accounts (political). *Contas de gestão*: accounts of the official as ordenador de despesas (managerial). The distinction matters for Ficha Limpa — see [`contas-municipais.md`](contas-municipais.md).
- **contestação** — Defendant's answer / defense brief in civil proceedings.
- **corregedoria** — Internal disciplinary body within a tribunal or MP. Investigates judges/promotores.

## D

- **decadência** — Automatic expiration of a substantive right. Distinguish from *prescrição*, which extinguishes the procedural right to enforce a claim.
- **decisão monocrática** — Single-judge decision. Contrast with *acórdão* (collegiate decision).
- **decreto** — Executive decree (regulamentar or autonomous). *Decreto-lei (DL)*: pre-1988 legislative instrument with statute force, abolished by CF/88 but DL-numbered statutes (CP, CPP, CLT) remain in force.
- **denúncia** — Formal criminal complaint by the MP, marking the start of the public criminal action. Distinguish from *queixa-crime* (private criminal action).
- **desembargador** — Second-instance judge in TJ/TRF/TRT. Sits on appellate chambers (câmaras, turmas) of state, federal, or labor courts. Not a *ministro* (which is reserved for superior courts).
- **despacho** — Interlocutory order — administrative or procedural, not adjudicative on the merits.
- **direito indisponível** — A right that cannot be waived even voluntarily. In labor law, many CLT protections are *indisponíveis* (Art. 611-B); a worker cannot validly contract them away. → [`justica-trabalho.md`](justica-trabalho.md)
- **dispensa** — Exemption from competitive bidding (procurement). *Dispensa de licitação* permits direct contracting in enumerated cases (Lei 8.666 Art. 24, Lei 14.133 Art. 75).
- **dolo / doloso** — Intent / intentional. Contrast with *culpa / culposo* (negligence). Post-Lei 14.230/2021 reform, only dolosa improbidade is actionable.
- **domicílio eleitoral** — Voter's electoral residence; the district where they vote. Candidates must have electoral domicile in the district where they run.

## E

- **ementa** — Headnote or abstract. For a statute, a one-sentence summary of its purpose appearing right after the law number/date. For a judgment, the editor-prepared summary of holdings.
- **enriquecimento ilícito** — Illicit enrichment; one of the three categories of improbidade (Art. 9 Lei 8.429/92).
- **entrância** — Career tier within the MP and judiciary. Inicial → intermediária → final. Small comarcas are inicial; state capital is final. Promotion between entrâncias is by *antiguidade* or *merecimento*.
- **estabilidade** — Job tenure for civil servants. Acquired after 3 years (CF Art. 41 §4) for concursados.
- **execução** — Enforcement phase of a case. *Fase de execução*: post-judgment phase where the judgment is enforced. *Execução fiscal*: tax enforcement (the largest source of judicial congestion in Brazil).

## F

- **fiscal da lei** — A party (typically the MP) acting as guardian of the law in a proceeding it didn't initiate, ensuring legality and procedural fairness.
- **foro privilegiado** — Special trial venue based on the defendant's office. Politicians and high officials have foro at higher courts for specific case types. Does NOT generally apply to improbidade actions. → [`procedimentos-legais.md`](procedimentos-legais.md)
- **fracionamento** — Splitting of procurement to evade modality thresholds. Prohibited (Lei 8.666 Art. 23 §5; Lei 14.133 Art. 75 §1).

## G

- **garantias** — Constitutional guarantees of independence. For judges and MP members: *vitaliciedade*, *inamovibilidade*, *irredutibilidade de subsídio*. → [`juizes.md`](juizes.md), [`funcoes-essenciais.md`](funcoes-essenciais.md)

## H

- **habeas corpus (HC)** — Constitutional remedy against actual or imminent unlawful restraint of liberty. Can be filed by anyone for any beneficiary; no fee.
- **habeas data** — Constitutional remedy to access or correct personal data held by a public-records body.
- **homologação** — Judicial validation. A *sentença homologatória* judicially endorses an agreement (e.g., a settlement) and gives it executory force.

## I

- **impedimento** — Formal disqualification of a judge or member of MP based on objective grounds (e.g., relationship to a party). It is *matéria de ordem pública*: the judge can recognize it ex officio. Distinguish from *suspeição* (subjective bias), which must be raised by a party.
- **improbidade administrativa** — Civil corruption / breach of administrative integrity, governed by Lei 8.429/1992. → [`improbidade.md`](improbidade.md)
- **improcedente / procedente** — Case outcomes. *Procedente*: granted on the merits (plaintiff wins). *Improcedente*: denied on the merits (defendant wins). *Extinto sem julgamento de mérito*: dismissed without reaching the merits.
- **inamovibilidade** — Irremovability. Constitutional guarantee that a judge or MP member cannot be transferred without consent, except by majority decision of the órgão colegiado on grounds of public interest.
- **inelegibilidade** — Ineligibility to run for elected office. Triggered by Ficha Limpa hypotheses, electoral abuse convictions, etc. Generally lasts 8 years from the triggering event. → [`partidos-e-sistema-eleitoral.md`](partidos-e-sistema-eleitoral.md), [`contas-municipais.md`](contas-municipais.md)
- **inquérito civil (IC)** — MP investigative proceeding for non-criminal matters. Exclusive to MP.
- **inquérito policial (IP)** — Police investigation in criminal matters.
- **inexigibilidade** — Procurement exemption when competition is objectively infeasible (sole source, artistic services, etc.). Distinguish from *dispensa* (where competition would be possible but is waived).

## J

- **juiz titular** — Chief judge of a *vara*; permanently assigned. *Juiz substituto*: substitute judge attached to the same vara.
- **juízo / juízos** — Court / jurisdictional unit. Often used loosely to mean "the court".
- **jurisprudência** — Body of court precedents. Brazilian use is broader than US "case law" — includes both binding and persuasive authority.

## L

- **laranja** — Straw person; a passive nominee who lends their name/ID to disguise the real party in interest. → [`anticorrupcao-penal.md`](anticorrupcao-penal.md)
- **lei complementar (LC)** — Statute requiring an absolute majority of each house (vs. simple majority for ordinary leis). Reserved for matters the Constitution explicitly assigns to LC.
- **lei orgânica** — Organic law that constitutes an institution. *Lei orgânica municipal*: a municipality's "constitution".
- **liminar** — Preliminary injunction. Granted in urgent cases (perigo + plausibility of right).
- **lista tríplice** — Triple list. A list of three names submitted by a body (MP, OAB) from which a higher authority chooses an appointee. Common in PG and tribunal selections.

## M

- **mandado de segurança** — Constitutional writ for the protection of clear, individual rights against abusive acts by public authority. Distinguish from *ação ordinária* (which can litigate broader civil claims).
- **matéria de ordem pública** — Question of public order; can be raised by the judge ex officio at any stage. *Impedimento* is matéria de ordem pública; *suspeição* is not.
- **merecimento** — Merit. One of two criteria for career advancement (the other is *antiguidade*).
- **ministro** — Justice. Reserved for members of superior courts (STF, STJ, TST, TSE, STM, TCU). Lower-court judges are *juízes* or *desembargadores*.
- **monocrático** — Single-judge (vs. colegiado/turma). A *decisão monocrática* is rendered by one judge sitting alone.

## N

- **ne bis in idem** — Prohibition on double jeopardy. In Brazilian law, does not generally bar parallel proceedings across different regimes (criminal, civil, administrative, electoral) — see [`improbidade.md`](improbidade.md) and [`anticorrupcao-penal.md`](anticorrupcao-penal.md).
- **nexo causal** — Causal nexus. The link between conduct and result that supports liability. In labor accident cases the **NTEP** establishes a statistical nexo.

## O

- **OAB** — Ordem dos Advogados do Brasil. Bar association.
- **ordenador de despesas** — Expense administrator; the official legally authorized to authorize public spending and bound by it. The mayor as ordenador is responsible for *contas de gestão*.
- **órgão competente** — Competent body / organ. In Ficha Limpa context (LC 64/90 Art. 1 I "g"), the câmara municipal is the *órgão competente* for judging mayoral accounts.

## P

- **parecer** — Advisory opinion. *Parecer prévio* of the TCE on annual mayoral accounts is sent to the câmara, which votes whether to follow it.
- **parecer prévio** — TCE preliminary opinion on annual contas de governo. Prevails unless 2/3 of vereadores override (CF Art. 31 §2). → [`contas-municipais.md`](contas-municipais.md)
- **permuta** — Mutual swap between two members of a career (judges, promotores) — both move to each other's positions.
- **petição inicial** — Initial complaint that launches a civil action.
- **prazo / prazos** — Deadline / time limits. Procedural prazos are generally counted in *dias úteis* (business days) post-CPC 2015.
- **pregão** — Reverse-auction modality of public procurement (Lei 10.520/2002). Pregão *eletrônico* is now the dominant federal modality. → [`licitacoes.md`](licitacoes.md)
- **prescrição** — Statute of limitations on procedural enforcement. Distinguish from *decadência* (expiration of substantive right).
- **prevaricação** — Willful failure of a public official to act on a duty or to act improperly, motivated by personal interest or feeling. CP Art. 319.
- **prevenção** — Prior assignment that fixes future jurisdiction. Once a relator is assigned a case, prevenção makes them the "julgador certo" for all subsequent acts in the same processo.
- **processo** — Proceeding / case. Used for judicial, administrative, and TC processes.
- **procurador / procuradora** — Member of MP at second instance. *Procurador da República*: MPF first instance member. *Procurador de Justiça*: MPE second instance. *Procurador do Estado/Município*: state/municipal attorney (advocacia pública), NOT MP.
- **promotor / promotora** — Member of MP at first instance. *Promotor de Justiça* in MPE; *Procurador da República* in MPF.

## Q

- **quitação** — Discharge; release from an obligation. *Quitação anual*: annual discharge of labor obligations between employer and employee before the union (Art. 507-B CLT, post-2017 reform).
- **quorum** — Quorum. Simple majority generally suffices for follow-the-TCE decisions; **2/3 of all members** required to override the parecer prévio (CF Art. 31 §2).

## R

- **ratificação** — Validation of an executive act by a higher authority.
- **recurso** — Appeal. *Recurso especial* (REsp) → STJ on federal-law questions. *Recurso extraordinário* (RE) → STF on constitutional questions. *Recurso ordinário*: ordinary appeal between instances.
- **relator** — Rapporteur. The judge or conselheiro to whom a case is initially assigned. Drafts the opinion, leads discussion, and is generally the *prevento* judge for all related future acts.
- **remoção** — Permanent transfer within the same entrância. Distinguish from *promoção* (between entrâncias) and *permuta*.
- **repercussão geral** — STF's docket-control mechanism. To be heard via RE, a case must be recognized as having "general repercussion" (CF Art. 102 §3, EC 45/2004). Once recognized, the resulting decision (a *Tema*) binds lower courts.
- **ressarcimento** — Restitution / reparation of damages. In improbidade, ressarcimento is *imprescritível* for dolosa conduct (STF Tema 897).

## S

- **sanção** — Sanction; penalty. In legislative context, also the President's act of approving and promulgating a law.
- **segredo de justiça** — Judicial secrecy. Specific case categories (family law, certain criminal investigations, juveniles) are restricted from public access despite the general publicity rule (CF Art. 93 IX).
- **sentença** — First-instance judgment. Distinguish from *acórdão* (collegiate decision).
- **sigilo** — Secrecy / confidentiality. *Quebra de sigilo* refers to breaching tax, banking, or telecom secrecy via judicial order.
- **sobras** — Remaining seats in proportional elections, distributed among parties that met the quociente eleitoral via successive quotient. → [`partidos-e-sistema-eleitoral.md`](partidos-e-sistema-eleitoral.md)
- **sorteio** — Random draw. Used for case assignment in many courts and TCE relator assignment.
- **subsídio** — The single-payment compensation of judges, MP members, and elected officials (no separate "salary" plus "benefits").
- **súmula** — Non-binding caselaw summary issued by a superior court (STF, STJ, TST, etc.). A short statement of the court's settled position.
- **súmula vinculante** — Binding precedent issued by the STF (CF Art. 103-A, EC 45/2004). Distinguish from regular *súmula*: only súmulas vinculantes formally bind lower courts.
- **suspeição** — Suspicion of bias of a judge or MP member based on subjective grounds. Must be raised by a party (not matéria de ordem pública). Distinguish from *impedimento*.

## T

- **tese** — Proposition of law. Used in two main contexts: (1) the formal *tese* set by STF in a Tema (e.g., "tese de Tema 157"), which binds lower courts; (2) the parties' *tese* in a case (their legal theory).
- **testa de ferro** — Front man / nominee. Active version of *laranja* — manages assets or contracts on behalf of a hidden principal. → [`anticorrupcao-penal.md`](anticorrupcao-penal.md)
- **trânsito em julgado** — Final and unappealable judgment. The moment a decision becomes coisa julgada and can no longer be challenged via ordinary appeal.
- **turno / segundo turno** — Round of an election. Second turno applies to majoritário offices (President, Governador, Prefeito) when no candidate gets 50%+1 in the first round (and only in municipalities >200k voters for prefeito).

## V

- **vara** — Court unit. The smallest jurisdictional unit, headed by a juiz titular. Located within a comarca (state) or seção judiciária (federal) or foro trabalhista (labor).
- **vereador / vereadora** — Municipal councillor. Elected to the câmara municipal.
- **vista** — Procedural pause to allow review. *Pedido de vista*: request by a judge to take time before voting.
- **vitaliciedade** — Tenure for life (after 2 years of probation), with removal only by judicial action. Constitutional guarantee for judges and MP members.

---

## See also

- [`siglas.md`](siglas.md) — acronym registry
- [`leis_index.yaml`](leis_index.yaml) — statutory index
- [`jurisprudencia-stf.md`](jurisprudencia-stf.md) — STF case file
